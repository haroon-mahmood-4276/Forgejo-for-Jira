...
Releases go through the `release` agent (`.claude/agents/release.md`) and nowhere
else. Do not tag a version, publish a GitHub release, or run `forge deploy -e production`
by hand — invoke that agent instead. It decides major/minor/patch from the commits,
owns the `vN` branch rule, and is the only thing that should touch production.

Deploying to `development` while working is normal and is not a release.
